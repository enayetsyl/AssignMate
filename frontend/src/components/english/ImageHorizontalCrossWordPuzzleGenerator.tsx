...
const ImageHorizontalCrossWordPuzzleGenerator: React.FC = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const computedPuzzles: PuzzleFinal[] = puzzles.map((puzzle, index) => {
      ...
      // Ensure the word fits horizontally in a 16-cell row.
  ...
  // For each cell, if it belongs to any puzzle’s placement, we mark it with a white background.
  // If it’s the starting cell, we display the sequential puzzle number.
  const renderGrid = () => {
    const cells = [];
    for (let r = 0; r < 16; r++) {
      for (let c = 0; c < 16; c++) {
        ...
        // Check if this cell is part of any puzzle’s placement.
        finalPuzzles.forEach((puzzle) => {
          const { row, col, wordLength } = puzzle.gridInfo;
          if (r === row && c >= col && c < col + wordLength) {
            bgColor = 'bg-white';
            // If it's the starting cell, display the puzzle number.
            if (c === col) {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h1 className="text-2xl font-bold mb-4">Crossword Puzzle App</h1>
      ...
      {finalPuzzles.length > 0 && (
        <div id="printable-area">
          ...
          <div className="mt-8">
            <h2 className="text-xl font-semibold mb-4">Write the image name in the appropriate box</h2>
            <div className="grid grid-cols-5 gap-4">
              {finalPuzzles.map((puzzle) => (
                ...
                >
                  ...
                  {/* <p className="mt-1">Word: {puzzle.word}</p> */}
                </div>
              ))}
            </div>
          </div>
...
export default ImageHorizontalCrossWordPuzzleGenerator;
