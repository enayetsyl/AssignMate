...
interface SelectedQuestionData {
  question: ShadowImageMatching;
}

type SelectedQuestionMap = Record<string, SelectedQuestionData>;

const ShadowImageMatching = () => {
  const [questions, setQuestions] = useState<ShadowImageMatching[]>([]);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await fetch(`${baseUrl}/shadow-image-matching?page=${currentPage}&limit=${limit}`);
        const data = await res.json();
        setQuestions(data.questions);
        setTotal(data.total);
      } catch (error) {
        console.error('Failed to fetch shadow matching questions:', error);
      }
    };
    fetchQuestions();
  }, [currentPage, baseUrl]);
  ...
  const handlePrintSelected = () => {
    ...
    const htmlContent = `
      <html>
        <head>
          <title>Print Shadow Matching Question</title>
        ...
        <body>
          <div class="print-container">
            <div class="title">
              Find ${question.answer} shadow matches
            </div>
            <div class="image-container">
              <img src="${question.image}" alt="Shadow Matching Image" />
            </div>
          </div>
        </body>
      </html>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      <h1 className="text-3xl font-bold mb-4">Shadow Matching Quiz</h1>
      {questions.length === 0 ? (
        <p>Loading shadow matching questions...</p>
      ) : (
          ...
          {questions.map((q, index) => (
            <div key={q._id} className="p-4 border rounded flex items-center space-x-4">
              ...
              <div className="flex-shrink-0">
                <Image src={q.image} alt={`Shadow Matching ${index + 1}`} height={40} width={40} className="w-40 h-40 object-cover border" />
              </div>
              <div className="flex items-center space-x-2">
                <label className="flex items-center">
                  <Input type="radio" name="selectedShadowMatching" checked={selectedQuestionId === q._id} onChange={() => handleSelectQuestion(q._id)} className="mr-2" />
                  <span>Select for Print</span>
                </label>
                <div className="flex items-center space-x-1">
                  <Label htmlFor={`shadowMatching-answer-${q._id}`}>Matches:</Label>
                  <Input
                    id={`shadowMatching-answer-${q._id}`}
                    type="number"
      ...
      <div className="mt-6">
        <Button type="button" onClick={handlePrintSelected} className="bg-green-500 text-white px-4 py-2 rounded">
          Print Selected Shadow Matching
        </Button>
      </div>
...
export default ShadowImageMatching;
