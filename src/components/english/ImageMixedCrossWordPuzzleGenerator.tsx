...
// Create an empty 16×16 grid (we still use word length behind the scenes)
...
/**
 * Write the word’s letters into the grid (used internally, but not displayed).
 */
 ...
 * try aligning so they overlap. If a valid alignment is found, return {row, col}.
 */
...
const ImageMixedCrossWordPuzzleGenerator: React.FC = () => {
  ...
  // 3) The internal letter grid (used for placement logic only)
  ...
  // Handle word input (we only use the word length for placement)
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    // Clear old grid and placement data.
    ...
    puzzles.forEach((puzzle, index) => {
      ...
      // Orientation: even index = horizontal, odd index = vertical.
      ...
      if (index === 0) {
        ...
        // For subsequent puzzles, first attempt forced overlap.
        ...
        if (overlapPos) {
          ...
          // If forced overlap fails, try a new cluster.
          const freshPos = tryPlaceAsNewCluster(newGrid, word, orientation);
          if (freshPos) {
            startRow = freshPos.row;
            startCol = freshPos.col;
  ...
  // - Default cells (with no puzzle) are gray.
  // - Cells that belong to a puzzle are white.
  // - Only the starting cell shows the puzzle number; no letters are shown.
  const renderGrid = () => {
    const cells = [];
    for (let r = 0; r < GRID_SIZE; r++) {
      for (let c = 0; c < GRID_SIZE; c++) {
  ...
  // Print only the content inside #printable-area.
  const handlePrint = () => {
    window.print();
  };

  // Separate puzzles by orientation for image layout.
    ...
    <div className="container mx-auto p-4">
      ...
      {finalPuzzles.length > 0 && (
        <div id="printable-area" className="mt-8">
          ...
          {/* Middle row: grid on the left, vertical puzzle images on the right */}
